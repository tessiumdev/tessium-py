...
_TERMINAL = frozenset({"unauthorized", "key_revoked", "account_banned", "account_deleted"})
...
class Client:
    ...
    def _receive(self, frame: dict[str, Any]) -> None:
        ...
        if op == "ack":
            ...
            return
        if op == "notice":
            if frame.get("code") in _TERMINAL:
        ...
        if op == "error":
            ...
            entry = self._state.subs.get(name) if frame.get("feature") == "cursor" and name else None
            if entry is not None:
                entry.resumable = False
                entry.cursor = None
                asyncio.create_task(self._send_subscribe(name, entry))  # type: ignore[arg-type]
                return
            self._fail(TessiumError(frame, name))
